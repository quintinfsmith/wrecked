...
for target_opt, target_name, platname in targets:
    ...
    os.system("cp target/%s/release/libwrecked_bindings.so %s/wrecked/libwrecked_%s.so" % (target_name, folder, platname))
...
if "--zip" in sys.argv:
    ...
    os.chdir("../")
    #os.system("cp target/%s/release/libwrecked_bindings.so ~/.local/lib/libwrecked.so" % target_name)
else:
    input()
    os.chdir("../")
